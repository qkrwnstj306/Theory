...
- 100% 스캐너블 보장 불가: 제안한 방법이 뛰어나긴 하지만, 생성된 aesthetic QR 코드가 항상 100% 스캐너블하다고 보장하지는 않습니다.
- 하이퍼파라미터 조정 필요: 최적의 결과를 위해서는 하이퍼파라미터 조정이 필요하며, 이 부분이 사용에 불편함을 초래할 수 있습니다.

- 논문에서는 이러한 문제를 해결하기 위해 향후 연구에서는  
  - 하이퍼파라미터에 덜 민감한(end-to-end) 파이프라인 개발  
  - 개인의 취향에 맞는 aesthetic QR 코드를 위한 이미지-투-이미지(image-to-image) 방식 활용
