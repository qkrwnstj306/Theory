...
$$ \hat{\theta}_{MLE} = \argmax_{\theta}p(\theta|D) = \argmax_{\theta}{\frac{p(D|\theta)p(\theta)}{p(D)}} $$

- $\theta$ 에 대해서 $p(D)$ 는 상수 취급이므로, 제거할 수 있다.

$$ \hat{\theta}_{MLE} \propto \argmax_{\theta}{p(D|\theta)p(\theta)} $$

- $\log$ 를 씌우면,

$$ \hat{\theta}_{MLE} \propto \argmax_{\theta}{\log{p(D|\theta)} + \log{p(\theta)}} $$
